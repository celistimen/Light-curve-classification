...
class CacheManager():
    def __init__(self, name = "tsfresh"):
        self.name = name
        self.cache_path = 'cachedir/' + name + '_cache'
        self.cache = {}
        
        if (name == "tsfresh"):
            self.extraction_pipe = make_pipeline(
                    FunctionTransformer(Utils.DfToNumpy, validate = False),
                    FE.Tsfresh()
                )
        elif (name == "TSFEL"):
            self.extraction_pipe = make_pipeline(
    ...
    #         if (all(number in dic for number in lijst))
        ...
        if (len(self.cache) == 0):
            features = self.extraction_pipe.transform(df)
    ...
    def add(self, df):
        extracted = self.extraction_pipe.transform(df)
